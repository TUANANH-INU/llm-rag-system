...
class OllamaLLM:
    ...
    def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 512, stream: bool = False) -> str:
        """
        ...
        try:
            response = requests.post(
                ...
                json={"model": self.model, "prompt": prompt, "temperature": temperature, "num_predict": max_tokens, "stream": stream},
                timeout=300,
            )
    ...
    def generate_with_context(self, query: str, context: List[Dict[str, Any]], temperature: float = 0.7, max_tokens: int = 256) -> tuple:
        """
...
class RAGPipeline:
    ...
    def __init__(self, llm_model: str = "llama2", embedding_model: str = "nomic-embed-text"):
